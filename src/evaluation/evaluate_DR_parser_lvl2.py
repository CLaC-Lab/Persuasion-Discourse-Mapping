"""
This module evaluates Level-2 discourse relation (DR) predictions made by multiple LLM-based parsers.

It performs the following:
- Calculates F1 scores (macro, micro, weighted) and accuracy for each parser-prompt combination.
- Computes consistency of parser outputs across two trials.
- Aggregates and saves detailed and summary performance reports as Excel, JSON, and TXT files.
- Visualizes performance with bar plots and multi-metric comparison charts.

The evaluation results help compare models like GPT, Gemini, and Claude on their DR prediction reliability.
"""
...
extended_metrics_comparison_path = os.path.join("..", "results", "stage1_eval_initial_DR_prompts_126instances", "summary_report", "extended_metrics_comparison.png")


def assign_numeric_id(prompt_id):
    """Extracts numeric ID from a prompt string like 'promptN03'."""
...
def calculate_parser_consistency(dataset_path):
    """Computes agreement percentage between two trials for each parser."""
...
def plot_metrics_subplots(df_avg):
    """Plots F1, hallucination, and consistency metrics across models and prompts."""
    ...
    plt.tight_layout()
    plt.savefig(extended_metrics_comparison_path, format='png', dpi=300, bbox_inches='tight')
    plt.close()


def evaluate_DR_parser_with_f1(): 
    """Evaluates DR predictions using F1 scores and accuracy, and generates reports and plots."""
