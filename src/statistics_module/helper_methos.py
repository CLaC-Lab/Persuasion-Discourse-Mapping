"""
This module contains visualization and analysis utilities for exploring the relationship
between Persuasion Techniques (PT) and Discourse Relations (DR) across pooled datasets.

Functions include:
- Drawing a heatmap of statistically significant PT-DR associations.
- Computing frequency tables for PT-DR co-occurrence.
- Saving global significance results.
- Calculating and visualizing Pointwise Mutual Information (PPMI) heatmaps.

Outputs include Excel files and PNG heatmaps, saved under the statistical analysis results directory.
"""
...
def draw_pt_dr_association_heatmap(global_significance_pairs_dict):
    """
    Draws and saves a heatmap showing how often each PT-DR pair is significantly associated
    across multiple datasets. Associations are determined by statistical tests (p ≤ 0.05, OR > 1).

    Args:
        global_significance_pairs_dict (dict): Mapping of (PT, DR) pairs to stats and dataset occurrences.
    """
...
def create_freq_table(data, freq_threshold):
    """
    Computes PT-DR co-occurrence frequency table, including totals and filters by frequency thresholds.

    Args:
        data (dict): Dictionary of annotated samples with 'PT' and 'DR' keys.
        freq_threshold (int): Minimum count required to retain a PT or DR in the table.

    Returns:
        tuple: (frequency table DataFrame, DR frequency dict, PT frequency dict,
                PT-DR co-occurrence dict, total co-occurrence count)
    """
...
def create_global_ppmi_heatmap(ppmi_global_table):
    """
    Saves the global average PPMI table to Excel and generates a heatmap image.

    Args:
        ppmi_global_table (DataFrame): Aggregated PPMI scores across datasets.
    """
