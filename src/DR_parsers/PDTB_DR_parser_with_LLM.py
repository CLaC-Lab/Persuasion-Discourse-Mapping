...
input_file = os.path.join(dataset_path, "02_processed_data", "discourse_relations", "PDTB3_126.json") 
output_file = os.path.join("..","results", "stage1_eval_initial_DR_prompts_126instances", "json_DR_level2_predictions", f"{prompt_key}_{model_name}_t1.json")


def parser_DR_with_LLM():
    """
    Applies a selected LLM parser to predict DR labels for each sample in the PDTB3 dataset.

    Uses the prompt template from config and saves predictions to a JSON file.
    """
    print('''
